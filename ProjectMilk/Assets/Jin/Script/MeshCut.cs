...
public class MeshCut
{
    ...
    public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial)
    {
        ...
        blade = new Plane(
            victim.transform.InverseTransformDirection(-normalDirection),//ワールドからローカルへ「向き」を変更
            victim.transform.InverseTransformPoint(anchorPoint)//ワールドからローカルへ「ポジション」を変更
        );
        ...
        for (int sub = 0; sub < victim_mesh.subMeshCount; sub++)
        {
            ...
            for (int i = 0; i < indices.Length; i += 3)
            {
                ...
                sides[0] = blade.GetSide(victim_mesh.vertices[p1]);//平面の表がどちらを向いているかを判断
                sides[1] = blade.GetSide(victim_mesh.vertices[p2]);//法線が向いている方にいる点がture
